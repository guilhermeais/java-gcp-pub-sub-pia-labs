...
public class PostgresOrdersRepository implements OrdersRepository {
  ...
  public void saveOrder(Order order) throws SQLException {
    ...
    try {
      ...
      String sql =
        "INSERT INTO orders (uuid, created_at, processed_at, type, customer_id) VALUES (?, ?, ?, ?, ?)";
      PreparedStatement pstmt = conn.prepareStatement(sql);
      pstmt.setString(1, order.getUuid());
      pstmt.setTimestamp(
        2,
        new java.sql.Timestamp(order.getCreatedAt().getTime())
      );
      pstmt.setTimestamp(
        3,
        new java.sql.Timestamp(order.getProcessedAt().getTime())
      );
      pstmt.setString(4, order.getType());
      pstmt.setInt(5, order.getCustomer().getId());
      System.out.println("Sending SQL: " + pstmt.toString());
      pstmt.executeUpdate();
      ...
      System.out.println(
        "Error on saving order " + order.getUuid() + ": " + e.getMessage()
      );
  ...
  private void upsertCostumer(Connection conn, Customer customer)
    throws SQLException {
    try {
      System.out.println("Upserting customer " + customer.getId() + "...");
      String sql =
        "INSERT INTO customers (id, name) VALUES (?, ?) ON CONFLICT (id) DO UPDATE SET name = ?";
      PreparedStatement pstmt = conn.prepareStatement(sql);
  ...
  private void saveItems(Connection conn, List<Item> items, String orderUuid)
    throws SQLException {
    try {
      System.out.println("Saving items for order " + orderUuid + "...");

      String sql =
        "INSERT INTO items (id, order_uuid, product_id, quantity, category_id) VALUES (?, ?, ?, ?, ?)";
      ...
      System.out.println(
        "Error on saving items for order " + orderUuid + ": " + e.getMessage()
      );
      throw e;
    }
  }
  ...
  private void upsertSku(Connection conn, Sku sku) throws SQLException {
    try {
      ...
      String sql =
        "INSERT INTO products (id, value) VALUES (?, ?) ON CONFLICT (id) DO UPDATE SET value = ?";
      ...
      System.out.println(
        "Error on upserting sku " + sku.getId() + ": " + e.getMessage()
      );
      throw e;
    }
  }

  private void upsertCategory(Connection conn, Category category)
    throws SQLException {
    try {
      System.out.println("Upserting category " + category.getId() + "...");

      String sql =
        "INSERT INTO categories (id, sub_category_id) VALUES (?, ?) ON CONFLICT (id) DO NOTHING";
      ...
      if (category.getSubCategory() != null) {
        System.out.println(
          "Upserting subcategory " + category.getSubCategory().getId() + "..."
        );
  ...
  @Override
  public List<Order> fetchOrders(FetchOrdersCommand fetchOrdersCommand)
    throws SQLException {
    Connection conn = null;
    try {
      System.out.println(
        "[PostgresOrdersRepository.fetchOrders] Fetching orders..."
      );
      conn = DatabaseConnectionPool.getConnection();
      StringBuilder sqlBuilder = new StringBuilder(
        "SELECT o.uuid, o.created_at, o.processed_at, o.type, c.id as customer_id, c.name as customer_name, i.product_id, p.value, i.quantity, i.category_id, cat.sub_category_id " +
        "FROM orders o " +
      ...
      System.out.println(
        "[PostgresOrdersRepository.fetchOrders] Query: " + pstmt.toString()
      );
      ...
      while (rs.next()) {
        ...
        System.out.println(
          "[PostgresOrdersRepository.fetchOrders] Orders fetched: " + uuid
        );
        ...
        if (order == null) {
          ...
          System.out.println(
            "[PostgresOrdersRepository.fetchOrders] Order created: " +
            order.getUuid()
          );
        }
        ...
        item.setCategory(
          Category
            ...
            .subCategory(
              Category.builder().id(rs.getString("sub_category_id")).build()
            )
            .build()
        );
        ...
        System.out.println(
          "[PostgresOrdersRepository.fetchOrders] Orders fetched: " +
          ordersMap.size()
        );
      }
      ...
      System.out.println(
        "[PostgresOrdersRepository.fetchOrders] Error on fetching orders: " +
        e.getMessage()
      );
      throw e;
    } finally {
